...
public class AssettypeResource extends SimpleObjectResource<Assettype> {
    ...
    public boolean validateReference(long assettypeId) throws StorageException {
        //String name = Simcard entity.getNetworkproviderid();
        Collection<Asset> result = storage.getObjects(Asset.class,
                new Request(
                        new Columns.All(),
                        new Condition.Equals("assettypeid", assettypeId)
                )
        );
        if (!result.isEmpty()) {
            return false;
